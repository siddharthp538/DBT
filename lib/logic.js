 ...
 */
 ...
 async function VerifyToken(tx){
    ...
    const gasAgency = await gasAgencyRegistery.get(tx.gasAgencyRegistrationId);
    if(genHash === lastHash){
        const lpgRegistery =  await getAssetRegistry('org.example.dbt.LPGCylinder');
        const lpg = await gasAgencyRegistery.get(tx.lpgCylinderUid);
        lpg.beneficiaryAadhaarNo = aadhar;       
        gasAgency.gasAgencyDeficitIncurred += lpg.marketPrice - (lpg.marketPrice*lpg.subsidyRate);
        if(!gasAgency.gasAgencyOrdersServed.length){
            gasAgency.gasAgencyOrdersServed=[aadhar];
        }
        else{
            gasAgency.gasAgencyOrdersServed.push(aadhar);
        }
        gasAgency.gasAgencyInventory = gasAgencyInventory - 1;
 ...
 */
 ...
 async function NotifyGovAuthority(tx){
     ...
     const govAuth  = await gasAgencyRegistery.get(tx.govAuthorityId);
 ...
 */
 ...
 async function RelaseFunds(tx){
     ...
     let subsidy = factory.newResource('org.example.dbt', 'LPGCylinder', r);
     const gasAgencyRegistery =  await getParticipantRegistry('org.example.dbt.GasAgency');
     const gasAgency = await gasAgencyRegistery.get(tx.gasAgencyRegistrationId);
